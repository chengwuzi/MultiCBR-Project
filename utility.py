...
class BundleTrainDataset(Dataset):
    def __init__(self, conf, u_b_pairs, u_b_graph, num_bundles, u_b_for_neg_sample, b_b_for_neg_sample, neg_sample=1, pos_bpr_modulation=None):
        self.conf = conf
        self.u_b_pairs = u_b_pairs
        self.u_b_graph = u_b_graph
        self.num_bundles = num_bundles
        self.neg_sample = neg_sample
        self.pos_bpr_modulation = pos_bpr_modulation
    ...
    def __getitem__(self, index):
        ...
        if self.pos_bpr_modulation is None:
            return torch.LongTensor([user_b]), torch.LongTensor(all_bundles)

        pos_bpr_modulation = torch.tensor(self.pos_bpr_modulation[index], dtype=torch.float32)
        return torch.LongTensor([user_b]), torch.LongTensor(all_bundles), pos_bpr_modulation
...
class Datasets():
    def __init__(self, conf):
        ...
        self.train_pos_bpr_modulation = self.get_bpr_weighted_pos_modulation(conf, u_b_pairs_train)
        ...
        self.bundle_train_data = BundleTrainDataset(
            ...
            b_b_for_neg_sample,
            conf["neg_num"],
            pos_bpr_modulation=self.train_pos_bpr_modulation,
        )
    ...
    def get_bpr_weighted_pos_modulation(self, conf, u_b_pairs_train):
        bpr_weight_conf = conf.get("bpr_weight_matrix", {})
        if not bpr_weight_conf.get("enabled", False):
            return None

        gamma = float(bpr_weight_conf.get("gamma", 0.0))
        resolved_weight_path = self.resolve_project_path(
            bpr_weight_conf.get("weight_path"),
            "bpr_weight_matrix",
        )
        ...
        print(
            f"Loading BPR weight matrix for {self.name}: "
        ...
        with open(resolved_weight_path, "r", encoding="utf-8") as f:
            for idx, pair in enumerate(u_b_pairs_train):
                ...
                if not line:
                    raise ValueError(
                        f"BPR weight file ended early at line {idx + 1}; expected {expected_lines} lines."
                    )

                parts = line.strip().split()
                if len(parts) != 3:
                    raise ValueError(f"Invalid BPR weight file format at line {idx + 1}: {line.strip()}")
                ...
                if pair[0] != user_id or pair[1] != bundle_id:
                    raise ValueError(
                        "BPR weight file is not aligned with user_bundle_train.txt at line "
                        f"{idx + 1}: train_pair={pair}, weight_pair=({user_id}, {bundle_id})"
                    )
            ...
            extra_line = f.readline()
            if extra_line:
                raise ValueError("BPR weight file has more lines than user_bundle_train.txt.")
        ...
        print(
            "BPR weight matrix transformed with "
